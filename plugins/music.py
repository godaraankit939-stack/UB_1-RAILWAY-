...
from telethon.tl.types import DataJSON, InputGroupCallConnection
...
async def setup(client):
    ...
    async def play_audio(event):
        ...
        try:
            ...
            await panel.edit(f"✨ **Found:** `{track_title}`\n🛰️ *Resolving Advanced Telethon Call Arguments...*")
        ...
        try:
            ...
            # Get bot's own self input entity to pass into 'join_as'
            my_self = await event.client.get_input_entity("me")

            # FIX FOR FUTURE METHOD: Enforcing mandatory 'join_as' and 'params' specifications
            ...
            await panel.edit(
                ...
                f"🔹 **Title:** `{track_title}`\n"
                f"👤 **Stream Provider:** `Assistant Account` (Updated Engine)"
            )
