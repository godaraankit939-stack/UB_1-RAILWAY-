...
# Yahan apne assistant ka exact username bina '@' ke config se fetch karega system
ASSISTANT_USERNAME = "DarkAssistantHomeBot"  # <-- Isko apne assistant username se badal lena agar alag ho
...
async def setup(client):
    ...
    async def professional_play_engine(event):
        # Framework authentication checks
        if await is_banned(event.sender_id): return
        if event.incoming:
            # Agar incoming message hai toh check karega ki chalanewala owner ya sudo hai ya nahi
            if event.sender_id != OWNER_ID and not await is_sudo(event.sender_id): return

        query = event.pattern_match.group(1)
        
        # UI response mapping based on who triggered it
        if event.outgoing:
        ...
        try:
            # STEP 1: Core Voice Chat Creation Layer (Forcefully auto-starts VC if not active)
            try:
                # Phle check karo ya try karo VC create karne ka, agar pehle se on hogi toh skip ho jayega
                await event.client(CreateGroupCallRequest(peer=event.chat_id))
                await asyncio.sleep(1) # Small delay for system server synchronization
            except Exception:
                pass # Already active or creation handle overridden

            # STEP 2: Fetch group call structural context safely via dynamic extraction
            try:
            ...
            if not call_info:
                return await panel.edit("❌ **Failed to initialize or find Voice Chat context in this group.**")

            # STEP 3: Professional Invite Bridge (Tujhe account join karne ki jarurat nahi, bot khud invite phekega)
            ...
            # STEP 4: Assistant Background Audio Injection Protocol Trigger
            # Note: Pyrogram session now listens to this dynamic broadcast stream link
            try:
                await assistant.join_chat(event.chat_id)
            except Exception:
                pass # Assistant inside target directory

            await panel.edit(
                f"🎵 **Now Streaming Live**\n\n"
                f"🔹 **Title:** `{track_title}`\n"
                f"👤 **Triggered By:** [Userbot Admin]\n"
                f"⚙️ **Status:** Professional Invite Routing Completed"
            )
