...
from telethon.errors import FlipValueError, FloodWaitError
...
async def setup(client):
    ...
    async def play_audio(event):
        ...
        try:
            ...
            my_self = await event.client.get_input_entity("me")

            # ADVANCED RETRY LAYER FOR SSRC HANDSHAKE BUGS
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    await event.client(JoinGroupCallRequest(
                        ...
                        muted=False,
                        video_stopped=True,
                        params=DataJSON(data='{"muted": false, "video_stopped": true}')
                    ))
                    break # Success huye hi loop break kar dega
                except Exception as e:
                    # Agar SSRC ya retry ka kalesh aaye toh thoda wait karke re-try hit karega
                    if "SSRC" in str(e) or "retry" in str(e).lower():
                        if attempt < max_retries - 1:
                            await asyncio.sleep(1.5)
                            continue
                    raise e # Agar aakhri baar bhi fail ho toh throw karega

            await panel.edit(
                f"🎵 **Assistant Streaming Live**\n\n"
                f"🔹 **Title:** `{track_title}`\n"
                f"👤 **Stream Provider:** `Assistant Account` (Auto-SSRC Fixed)"
            )
