...
# --- ADVANCED MULTI-PLATFORM AUDIO SCRAPER ---
async def fetch_direct_audio(query: str):
    """Bypasses constraints by dynamically switching between Saavn API and SoundCloud."""
                ...
                async with session.get(search_url, timeout=5) as resp:
                    if resp.status == 200:
                        ...
                        if res_data.get("success") and res_data["data"]["results"]:
                            ...
                            if download_urls:
                                return download_urls[-1]["url"], f"{top_track.get('name', 'Audio Track')} (JioSaavn)"
        except Exception:
            pass 
...
async def setup(client):
    ...
    async def play_audio(event):
        ...
        try:
            ...
            await panel.edit(f"✨ **Found:** `{track_title}`\n🛰️ *Resolving Peer Entity & Injecting Assistant...*")
        ...
        try:
            ...
            # CRITICAL ADVANCED FIX: Resolve pure peer input entity types to completely eliminate Object ID Crashes
            peer_entity = await event.client.get_input_entity(event.chat_id)
            
            if isinstance(peer_entity, InputPeerChannel):
                # Guaranteed execution path for Megagroups, Supergroups and Channels
                ...
                # Guaranteed execution path for Legacy/Normal Small Groups
                full_chat = await event.client(GetFullChatRequest(chat_id=peer_entity.chat_id))
                call_info = full_chat.full_chat.group_call
            else:
                # Absolute Fallback Layer using primitive fallback routing if structural casting shifts
                try:
            ...
            # Native Call Injection Bridge Execution
            ...
            await panel.edit(
                ...
                f"🔹 **Title:** `{track_title}`\n"
                f"👤 **Stream Provider:** `Assistant Account` (Future-Proofed)"
            )
