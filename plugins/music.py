...
async def setup(client):
    ...
    async def play_audio(event):
        ...
        try:
            ...
            # FIX: Get correct peer type dynamic mapping to prevent invalid object crashes
            peer = await event.client.get_input_entity(event.chat_id)
            
            if isinstance(peer, PeerChannel):
                # Megagroups / Supergroups / Channels structure extraction
                full_chat = await event.client(GetFullChannelRequest(channel=peer))
                call_info = full_chat.full_chat.group_call
            else:
                # Normal legacy small groups structure extraction
            ...
            await panel.edit(
                ...
                f"🔹 **Title:** `{track_title}`\n"
                f"👤 **Stream Provider:** `Assistant Account`"
            )
